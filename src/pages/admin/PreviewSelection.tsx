...
const PreviewSelection = () => {
    ...
    queryFn: async () => {
      ...
      if (equipeIds.length > 0) {
        const { data: membresData } = await supabase
          .from("membres")
          .select("equipe_id, nom_prenom, email, telephone")
  ...
  const generatePDF = async () => {
    ...
    doc.text("HACKATHON ISOC-ESMT 2026", 105, 25, { align: "center" });
    ...
    let contactY = 40;
    let contactIndex = 1;
    const allEquipes = equipesDetails || [];
    
    // ========== SECTION SÉLECTIONNÉS ==========
    const selectionnesEquipes = allEquipes.filter(e => e.statut === 'selectionne');
    ...
    if (selectionnesEquipes.length > 0) {
      ...
      doc.setFontSize(13);
      doc.setFont("helvetica", "bold");
      doc.text(`✓ SÉLECTIONNÉS (${selectionnesEquipes.length})`, 105, contactY + 8, { align: "center" });
      contactY += 18;
      ...
      for (const equipe of selectionnesEquipes) {
        const membres = equipe.membres || [];
        const isEquipe = equipe.type_candidature === 'equipe';
        ...
        if (isEquipe && membres.length > 0) {
          ...
          doc.setFontSize(9);
          doc.setFont("helvetica", "bold");
          doc.text(`ÉQUIPE: ${equipe.nom_equipe || 'Sans nom'}`, 15, contactY);
          contactY += 8;
        }
    ...
    // ========== SECTION LISTE D'ATTENTE ==========
    const attenteEquipes = allEquipes.filter(e => e.statut === 'en_attente');
    ...
    if (attenteEquipes.length > 0) {
      ...
      doc.setFontSize(13);
      doc.setFont("helvetica", "bold");
      doc.text(`⏳ LISTE D'ATTENTE (${attenteEquipes.length})`, 105, contactY + 8, { align: "center" });
      contactY += 18;
      ...
      for (const equipe of attenteEquipes) {
        const membres = equipe.membres || [];
        const isEquipe = equipe.type_candidature === 'equipe';
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      doc.text("Hackathon ISOC-ESMT 2026 - Innover, Créer, Entreprendre", 105, 290, { align: "center" });
