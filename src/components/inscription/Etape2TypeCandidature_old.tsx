          ...
          <label className="cursor-pointer">
            ...
            <div className={`relative overflow-hidden rounded-2xl border-2 p-6 transition-all duration-300 ${
              data.type_candidature === type.value
                ? 'border-[#FF6B35] bg-[#FF6B35]/5 shadow-lg shadow-[#FF6B35]/20'
                : 'border-[#E9ECEF] bg-white hover:border-[#FF6B35]/30 hover:shadow-md'
            }`}>
              {/* Selection indicator */}
              {data.type_candidature === type.value && (
                <motion.div
                  ...
                  className="absolute top-4 right-4 w-6 h-6 rounded-full bg-[#FF6B35] flex items-center justify-center"
                >
                  <motion.div
              ...
              <div className="flex items-center justify-center mb-4">
                <div className={`w-16 h-16 rounded-xl flex items-center justify-center ${
                  data.type_candidature === type.value
                    ? 'bg-gradient-to-r from-[#FF6B35] to-[#1E3A5F]'
                    : 'bg-gradient-to-r from-[#FF6B35]/10 to-[#1E3A5F]/10'
                }`}>
                  <type.icon 
                    size={32} 
                    className={data.type_candidature === type.value ? 'text-white' : 'text-[#FF6B35]'} 
                  />
                </div>
              </div>
              ...
              <div className="text-center">
                ...
                >
                  <span className={data.type_candidature === type.value ? 'text-[#FF6B35]' : 'text-[#212529]'}>
                    {type.title}
                  </span>
                </h4>
                <p 
                  ...
                  style={{ fontFamily: 'DM Sans, sans-serif' }}
                >
                  <span className={data.type_candidature === type.value ? 'text-[#1E3A5F]' : 'text-[#6C757D]'}>
                    {type.desc}
                  </span>
                </p>
                ...
                <div className="space-y-2">
                  {type.features.map((feature, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <Sparkles 
                        size={14} 
                        className={data.type_candidature === type.value ? 'text-[#FF6B35]' : 'text-[#6C757D]'} 
                      />
                      <span 
                        className="text-xs"
                        style={{ fontFamily: 'DM Sans, sans-serif' }}
              ...
              {/* Hover effect */}
              <div className="absolute inset-0 bg-gradient-to-r from-[#FF6B35]/5 via-transparent to-[#1E3A5F]/5 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-2xl"></div>
            </div>
          </label>
        </motion.div>
      ))}
